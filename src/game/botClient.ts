...
export interface BotClient {
  // Plan a shot off-thread. Resolves with the ShotInput to play.
  plan(request: Omit<BotPlanRequest, 'id'>): Promise<ShotInput>;
...
export const createBotClient = (): BotClient => {
  ...
  const pending = new Map<number, (shot: ShotInput) => void>();
  let nextId = 1;

  worker.onmessage = (event: MessageEvent<BotPlanResponse>): void => {
    const { id, shot } = event.data;
